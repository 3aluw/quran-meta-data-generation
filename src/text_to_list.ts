//This set of function used to transform a text array and approximate ayah ids -baseIds- (from other riwaya ) to a more accurate ayah ids list

import type {  AyahId } from 'quran-meta'
import { compareFirstLetters, } from './utils'
import { qulunData } from "./QalounData"
type comparisonType = "exact" | "levenshtein"

const baseIds: AyahId[] = [0,1,20,32,40,50,60,66,73,82,91,98,108,112,121,130,139,148,154,164,
174,183,191,195,203,208,218,224,233,238,241,248,253,258,264,269,273,278,288,289,
297,307,318,325,334,343,355,366,374,383,396,405,413,425,436,445,452,463,471,478,
487,498,503,505,511,516,523,528,538,549,555,565,569,578,584,591,596,605,614,621,
629,639,650,657,663,671,674,680,686,692,701,710,714,718,728,736,744,751,759,766,
775,780,790,803,815,826,840,849,859,870,881,885,890,901,910,917,926,931,935,941,
948,957,974,984,992,1002,1010,1020,1030,1043,1055,1072,1087,1098,1104,1112,1119,1127,1136,1144,
1155,1167,1174,1184,1194,1203,1212,1223,1235,1243,1250,1257,1263,1272,1278,1284,1292,1299,1308,1314,
1323,1332,1340,1350,1356,1362,1371,1379,1387,1394,1405,1417,1429,1439,1451,1461,1472,1483,1492,1501,
1509,1518,1527,1538,1550,1560,1571,1582,1596,1608,1620,1631,1641,1651,1665,1675,1686,1699,1708,1714,
1726,1730,1740,1745,1754,1766,1775,1783,1794,1808,1833,1856,1887,1907,1921,1936,1944,1957,1969,1977,
1985,1996,2004,2017,2026,2035,2045,2057,2070,2084,2095,2104,2119,2133,2145,2161,2167,2176,2188,2197,
2207,2218,2226,2239,2250,2270,2289,2308,2327,2349,2372,2402,2419,2429,2441,2456,2474,2483,2498,2512,
2524,2533,2555,2568,2582,2594,2604,2612,2621,2629,2640,2651,2661,2670,2690,2705,2727,2745,2767,2785,
2799,2809,2818,2823,2830,2839,2846,2849,2860,2871,2882,2895,2911,2928,2949,2977,3005,3038,3073,3108,
3135,3159,3171,3180,3195,3214,3222,3237,3249,3259,3267,3277,3286,3299,3309,3324,3331,3343,3351,3361,
3372,3382,3396,3413,3426,3435,3445,3458,3475,3485,3495,3508,3520,3533,3540,3547,3555,3562,3567,3578,
3582,3587,3599,3607,3615,3624,3634,3646,3659,3669,3685,3694,3701,3727,3744,3759,3776,3804,3833,3865,
3896,3927,3965,3984,3993,4015,4047,4059,4068,4081,4090,4100,4114,4123,4131,4143,4152,4163,4175,4188,
4200,4214,4222,4231,4243,4252,4260,4270,4278,4290,4299,4307,4320,4332,4351,4372,4393,4422,4446,4464,
4476,4491,4504,4510,4522,4535,4545,4558,4571,4581,4590,4593,4601,4606,4616,4637,4656,4686,4711,4737,
4757,4788,4817,4841,4864,4879,4909,4937,4984,5033,5061,5069,5075,5083,5090,5096,5108,5114,5128,5134,
5141,5155,5163,5174,5183,5190,5200,5206,5214,5219,5238,5268,5297,5339,5372,5393,5412,5428,5445,5473,
5504,5529,5568,5586,5614,5649,5689,5729,5766,5790,5830,5875,5896,5924,5959,6001,6036,6068,6104,6134,
6167]
const textArray: string[] =[
  "الحمد لله رب العلمين",
  "وإذا لقوا الذين آمنوا قالوا آمنا وإذا خلوا إلى",
  "إن الله لا يستحيي أن يضرب مثلا ما بعوضة",
  "وإذ قلنا للملائكة اسجدوا لآدم فسجدوا",
  "أتأمرون الناس بالبر وتنسون أنفسكم",
  "وإذ قال موسى لقومه يا قوم إنكم ظلمتم",
  "وإذ استسقى موسى لقومه فقلنا اضرب",
  "وإذ قال موسى لقومه إن الله يأمركم أن تذبحوا",
  "وإذا لقوا الذين آمنوا قالوا آمنا وإذا خلا بعضهم",
  "ثم أنتم هؤلاء",
  "ولقد جاءكم موسى بالبينات ثم اتخذتم العجل",
  "واتبعوا ما تتلوا الشياطين على ملك سليمان",
  "ما ننسخ من آية أو ننسها",
  "ولله المشرق والمغرب",
  "وإذ ابتلى إبراهيم ربه بكلمات فأتمهن",
  "أم كنتم شهداء إذ حضر يعقوب الموت",
  "سيقول السفهاء من الناس ما ولاهم عن قبلتهم",
  "ولكل وجهة هو موليها فاستبقوا الخيرات",
  "إن الصفا والمروة من شعائر الله",
  "يا أيها الناس كلوا مما في الأرض حلالا طيبا",
  "ليس البر أن تولوا وجوهكم قبل المشرق",
  "شهر رمضان الذي أنزل فيه القرءان",
  "يسألونك عن الأهلة قل هي مواقيت",
  "الحج أشهر معلومات",
  "واذكروا الله في أيام معدودات",
  "كان الناس أمة واحدة فبعث الله النبيئين",
  "يسألونك عن الخمر والميسر",
  "والمطلقات يتربصن بأنفسهن ثلاثة قروء",
  "والوالدات يرضعن أولادهن حولين كاملين",
  "لا جناح عليكم إن طلقتم النساء ما لم تمسوهن",
  "ألم تر إلى الذين خرجوا من ديارهم وهم ألوف",
  "وقال لهم نبيئهم إن آية ملكه أن يأتيكم التابوت",
  "تلك الرسل فضلنا بعضهم على بعض",
  "ألم تر إلى الذي حاج إبراهيم في ربه",
  "قول معروف ومغفرة",
  "يا أيها الذين آمنوا أنفقوا من طيبات ما كسبتم",
  "ليس عليك هداهم ولكن الله يهدي من يشاء",
  "يا أيها الذين آمنوا إذا تداينتم بدين",
  "وإن كنتم على سفر.",
  "إن الله لا يخفى عليه شيء",
  "قل أَٰوْ۟نَبِّئُكُم بخير من ذَٰلِكُمْۖ",
  "قل اللهم مالك الملك تؤتي الملك من تشاء",
  "إن الله اصطفى آدم ونوحا وآل إبراهيم",
  "وإذ قالت الملائكة يا مريم إن الله اصطفاك",
  "فلما أحس عيسى منهم الكفر قال من أنصاري",
  "قل يا أهل الكتاب تعالوا إلى كلمة سواء",
  "ومن أهل الكتاب من إن تأمنه بقنطار",
  "أفغير دين الله تبغون وله أسلم",
  "لن تنالوا البر حتى تنفقوا مما تحبون",
  "ولتكن منكم أمة يدعون إلى الخير",
  "ليسوا سواء من أهل الكتاب أمة قائمة",
  "وإذ غدوت من أهلك تبوئ المؤمنين",
  "سارعوا إلى مغفرة من ربكم وجنة عرضها",
  "وما محمد إلا رسول قد خلت من قبله الرسل",
  "إذ تصعدون ولا تلوون على أحد",
  "إن ينصركم الله فلا غالب لكم",
  "يستبشرون بنعمة من الله وفضل",
  "ما كان الله ليذر المؤمنين",
  "لتبلون في أموالكم وأنفسكم ولتسمعن",
  "فاستجاب لهم ربهم أني لا أضيع",
  "وابتلوا اليتامى حتى إذا بلغوا النكاح",
  "يوصيكم الله في أولادكم للذكر مثل حذ",
  "تلك حدود الله ومن يطع الله ورسوله",
  "يا أيها الذين آمنوا لا يحل لكم أن ترثوا",
  "والمحصنات من النساء إلا ما ملكت أيمانكم",
  "إن تجتنبوا كبائر ما تنهون نكفر عنكم سيئاتكم",
  "واعبدوا الله ولا تشركوا به شيئا",
  "يا أيها الذين أوتوا الكتاب آمنوا بما نزلنا",
  "إن الله يأمركم أن تؤدوا الأمانات",
  "وما أرسلنا من رسول إلا ليطاع بإذن الله",
  "فليقاتل في سبيل الله الذي يشرون",
  "أينما تكونوا يدرككم الموت",
  "الله لا إله إلا هو ليجمنعكم",
  "ومن يقتل مؤمنا متعمدا فجزاؤه جهنم",
  "ومن يهاجر في سبيل الله يجد",
  "إنا أنزلنا إليك الكتاب بالحق لتحكم بين الناس",
  "لا خير في كثير من نجواهم إلا",
  "ليس بأمانيكم ولا أماني أهل الكتاب",
  "وإن يتفرقا يغن الله كلا من سعته",
  "بشر المنافقين بأن لهم عذابا أليما",
  "لا يحب الله الجهر بالسوء من القول إلا",
  "وإن من أهل الكتاب إلا ليؤمنن به قبل موته",
  "لكن الله يشهد بما أنزل إليك أنزله بعلمه",
  "لن يستنكف المسيح أن يكون عبدا لله",
  "حرمت عليكم الميتة والدم ولحم الخنزير",
  "يا أيها الذين آمنوا إذا قمتم إلى الصلاة",
  "ولقد أخذ الله ميثاق بني إسرائيل وبعثنا",
  "لقد كفر الذين قالوا إن الله هو المسيح",
  "قال رجلان من الذين يخافون",
  "من أجل ذلك كتبنا على بني إسرائيل أنه",
  "يا أيها الرسول لا يحزنك الذين يسارعون",
  "وكتبنا عليهم فيها أن النفس بالنفس",
  "وأن احكم بينهم بما أنزل الله",
  "قل يا أهل الكتاب هل تنقمون منا إلا أن آمنا",
  "يا أيها الرسول بلغ ما أنزل إليك من ربك",
  "ما المسيح ابن مريم إلا رسول",
  "لتجدن أشد الناس عداوة للذين آمنوا",
  "يا أيها الذين آمنوا إنما الخمر والميسر",
  "جعل الله الكعبة البيت الحرام",
  "يا أيها الذين آمنوا شهادة بينكم إذا حضر",
  "وإذ أوحيت إلى الحواريين أن آمنوا بي",
  "الحمد لله الذي خلق السماوات والأرض",
  "وله ما سكن في الليل والنهار وهو السميع العليم",
  "ومنهم من يستمع إليك وجعلنا على قلوبهم",
  "إنما يستجيب الذين يسمعون",
  "قل لا أقول لكم عندي خزائن الله",
  "وعنده مفاتح الغيب لا يعلمها إلا هو",
  "وذر الذين اتخذوا دينهم لعبا ولهوا",
  "وحاجه قومه قال أتحاجوني في الله",
  "وما قدروا الله حق قدره إذ قالوا ما أنزل الله",
  "إن الله فالق الحب والنوى",
  "وجعلوا لله شركاء الجن وخلقهم",
  "ولو أننا نزلنا إليهم الملائكة وكلمهم الموتى",
  "وذروا ظاهر الإثم وباطنه",
  "لهم دار السلام عند ربهم وهو وليهم",
  "وجعلوا لله مما ذرأ من الحرث والأنعام نصيبا",
  "وهو الذي أنشأ جنات معروشات",
  "قل لا أجد في ما أوحي إلي محرما",
  "قل تعالوا أتلو ما حرم ربكم عليكم",
  "هل ينظرون إلا أن تأتيهم الملائكة",
  "ألمص كتاب أنزل إليك فلا يكن في صدرك",
  "ويا آدم اسكن أنت وزوجك الجنة",
  "قل أمر ربي بالقسط وأقيموا وجوهكم",
  "قال ادخلوا في أمم قد خلت من قبلكم",
  "وإذا صرفت أبصارهم تلقا أصحاب النار",
  "ادعوا ربكم تضرعا وخفية",
  "وإلى عاد أخاهم هودا قال يا قوم اعبدوا الله",
  "قال الملأ الذين استكبروا من قومه للذين",
  "قال الملأ الذين استكبروا من قومه لنخرجنك",
  "أولم يهد للذين يرثون الأرض",
  "وأوحينا إلى موسى أن ألق عصاك",
  "وقالوا مهما تأتنا به من آية لتسحرنا بها",
  "وواعدنا موسى ثلاثين ليلة",
  "واتخذ قوم موسى من بعده من حليهم",
  "واكتب لنا في هذه الدنيا حسنة",
  "وسئلهم عن القرية",
  "وإذ نتقنا الجبل فوقهم كأنه ظلة",
  "ولله الأسماء الحسنى فادعوه بها",
  "قل لا أملك لنفسي نفعا ولا ضرا إلا",
  "خذ العفو وأمر بالعرف",
  "كما أخرجك ربك من بيتك بالحق",
  "إذ يوحي ربك إلى الملائكة",
  "إن شر الدواب عند الله الصم البكم",
  "وإذ قالوا اللهم إن كان هذا هو الحق",
  "واعلموا أنما غنمتم من شيء",
  "إذ يقول المنافقون والذي في قلوبهم",
  "وأعدوا لهم ما استطعتم من قوة",
  "إن الذين آمنوا وهاجروا وجاهدوا",
  "فإذا انسلخ الأشهر الحرم فاقتلوا",
  "وإن نكثوا أيمانهم من بعد عهدهم",
  "أجعلتم سقاية الحاج",
  "لقد نصركم الله في مواطن كثيرة",
  "يا أيها الذين آمنوا إن كثيرا من الأحبار",
  "إلا تنصروه فقد نصره الله",
  "ولو أرادوا الخروج لأعدوا له عدة",
  "وما منعهم أن تقبل منهم نفقاتهم",
  "ومنهم الذين يؤذون النبيء ويقولون هو أذن",
  "ألم يأتهم نبأ الذين من قبلهم قوم نوح",
  "ومنهم من عاهد الله لئن آتانا من فضله",
  "ولا تصل على أحد منهم مات أبدا",
  "إنما السبيل على الذين يستأذنونك وهم أغنياء",
  "وممن حولكم من الأعراب منافقون",
  "إن الله اشترى من المؤمنين أنفسهم",
  "لقد تاب الله على النبيء والمهاجرين",
  "يا أيها الذين ءامنوا قاتلوا الذين يلونكم",
  "إن ربكم الله الذي خلق السماوات",
  "ولو يعجل الله للناس الشر استعجالهم",
  "وما كان الناس إلا أمة واحدة فاختلفوا",
  "للذين أحسنوا الحسنى وزيادة",
  "وما كان هذا القرآن أن يفترى من دون الله",
  "قل لا أملك لنفسي ضرا ولا نفعا إلا",
  "وما تكون في شأن وما تتلوا منه",
  "واتل عليهم نبأ نوح إذ قال لقومه يا قوم",
  "فما آمن لموسى إلا ذرية من قومه",
  "ولقد بوأنا بني إسرائيل مبوأ صدق",
  "قل يا أيها الناس إن كنتم في شك من ديني",
  "وما من دابة في الأرض إلا على الله رزقها",
  "من كان يريد الحياة الدنيا وزينتها",
  "مثل الفريقين كالأعمى والأصم",
  "قالوا يا نوح قد جادلتنا فأكثرت جدالنا",
  "وقال اركبوا فيها بسم الله مجراها",
  "وإلى عاد أخاهم هودا قال يا قوم اعبدوا الله",
  "قالوا يا صالح قد كنت فينا مرجوا قبل هذا",
  "فلما ذهب عن إبراهيم الروع",
  "وإلى مدين أخاهم شعيبا قال يا يقوم",
  "ولما جا أمرنا نجينا شعيبا والذين آمنوا",
  "يوم يأتي لا تكلم نفس إلا بإذنه",
  "وكلا نقص عليك من أنباء الرسل ما نثبت",
  "قال قآئل منهم لا تقتلوا يوسف وألقوه في غيابات الجب",
  "ولما بلغ أشده آتيناه حكما وعلما",
  "قال رب السجن أحب إلي مما يدعونني إليه",
  "وقال الملك إني أرى سبع بقرات سمان",
  "وما أبرئ نفسي إن النفس لأمارة بالسو",
  "وقال يا بني لا تدخلوا من باب واحد",
  "قالوا إن يسرق فقد سرق أخ له من قبل",
  "فلما دخلوا عليه قالوا يا أيها العزيز مسنا",
  "رب قد آتيتني من الملك وعلمتني",
  "حتى إذا استيأس الرسل وظنوا أنهم قد كذبوا",
  "وإن تعجب فعجب قولهم",
  "قل من رب السماوات والأرض قل الله",
  "أفمن يعلم أنما أنزل إليك من ربك الحق",
  "كذلك أرسلناك في أمة قد خلت من قبلها",
  "مثل الجنة التي وعد المتقون",
  "ألر كتاب أنزلناه إليك لتخرج الناس",
  "قالت رسلهم أفي الله شك فاطر",
  "ألم تر أن الله خلق السماوات والأرض",
  "ألم تر إلى الذين بدلوا نعمة الله كفرا",
  "الحمد لله الذي وهب لي على الكبر",
  "ألر تلك آيات الكتاب وقرآن مبين ربما يود",
  "ولقد خلقنا الإنسان من صلصال من حمإ",
  "نبئ عبادي أني أنا الغفور الرحيم",
  "ولقد كذب أصحاب الحجر المرسلين",
  "أتى أمر الله فلا تستعجلوه",
  "وألقى في الأرض رواسي أن تميد بكم",
  "وقيل للذين اتقوا ماذا أنزل ربكم",
  "وأقسموا بالله جهد أيمانهم لا يبعث الله",
  "وقال الله لا تتخذوا إلهين اثنين",
  "تالله لقد أرسلنا إلى أمم من قبلك فزين",
  "والله فضل بعضكم على بعض في الرزق",
  "ألم يروا إلى الطير مسخرات",
  "إن الله يأمر بالعدل والإحسان",
  "فإذا قرأت القرآن فاستعذ بالله",
  "يوم تأتي كل نفس تجادل عن نفسها",
  "إن إبراهيم كان أمة قانتا لله",
  "سبحان الذي أسرى بعبده ليلا",
  "ويدع الإنسان بالشر دعاءه بالخير",
  "وقضى ربك ألا تعبدوا إلا إياه",
  "ولا تقف ما ليس لك به علم",
  "قل كونوا حجارة أو حديدا",
  "وإذ قلنا للملائكة اسجدوا لآدم فسجدوا",
  "ولقد كرمنا بني آدم وحملناهم في البر",
  "ويسألونك عن الروح",
  "أولم يروا أن الله الذي خلق السماوات",
  "الحمد لله الذي أنزل على عبده الكتاب",
  "وترى الشمس إذا طلعت تزاور عن كهفهم",
  "فلا تمار فيهم إلا مراء ظاهرا",
  "واضرب لهم مثلا رجلين جعلنا لأحدهما",
  "واضرب لهم مثل الحياة الدنيا كماء أنزلناه",
  "ولقد صرفنا في هذا القرآن للناس من كل مثل",
  "قال ذلك ما كنا نبغ فارتدا على آثارهما قصصا",
  "قال ألم أقل لك إنك لن تستطيع معي صبرا",
  "ويسألونك عن ذي القرنين قل سأتلوا عليكم منه ذكرا",
  "وتركنا بعضهم يومئذ يموج في بعض",
  "كهيعص ذكر رحمة ربك عبده زكرياء",
  "فحملته فانتبذت به مكانا قصيا",
  "واذكر في الكتاب إبراهيم إنه كان صديقا نبيئا",
  "فخلف من بعدهم خلف أضاعوا الصلاة",
  "أفرأيت الذي كفر بآياتنا وقال لأوتين مالا وولدا",
  "طه ما أنزلنا عليك القرآن لتشقى",
  "قال رب اشرح لي صدري",
  "منها خلقناكم وفيها نعيدكم ومنها",
  "قالوا لن نؤثرك على ما جاءنا من البينات",
  "وما أعجلك عن قومك يا موسى",
  "قال فما خطبك يا سامري",
  "وعنت الوجوه للحي القيوم",
  "أفلم يهد لهم كم أهلكنا قبلهم من القرون",
  "اقترب للناس حسابهم وهم في غفلة",
  "وما خلقنا السماء والأرض وما بينهما لاعبين",
  "أولم ير الذين كفروا أن السماوات والأرض",
  "قل من يكلؤكم بالليل والنهار من الرحمن",
  "ولقد آتينا إبراهيم رشده من قبل",
  "ولوطا آتيناه حكما وعلما",
  "وذا النون إذ ذهب مغاضبا",
  "إن الذين سبقت لهم منا الحسنى",
  "يا أيها الناس اتقوا ربكم إن زلزلة الساعة",
  "ومن الناس من يعبد الله على حرف",
  "هذان خصمان اختصموا في ربهم",
  "ذلك ومن يعظم حرمات الله فهو خير له",
  "إن الله يدافع عن الذين آمنوا",
  "قل يا أيها الناس إنما أنا لكم نذير مبين",
  "ذلك ومن عاقب بمثل ما عوقب به",
  "ألم تعلم أن الله يعلم ما في السماء والأرض",
  "قد أفلح المؤمنون",
  "وإن لكم في الأنعام لعبرة",
  "هيهات هيهات لما توعدون",
  "إن الذين هم من خشية ربهم مشفقون",
  "ولو رحمناهم وكشفنا ما بهم من ضر",
  "وقل رب أعوذ بك من همزات الشياطين",
  "أفحسبتم أنما خلقناكم عبثا وأنكم",
  "إن الذين جاءوا بالإفك عصبة منكم",
  "يا أيها الذين آمنوا لا تتبعوا خطوات الشيطان",
  "قل للمؤمنين يغضوا من أبصارهم",
  "الله نور السماوات والأرض",
  "ألم تر أن الله يزجي سحابا ثم يؤلف بينه",
  "وأقسموا بالله جهد أيمانهم لئن أمرتهم ليخرجن",
  "والقواعد من النساء اللاتي لا يرجون نكاحا",
  "لا تجعلوا دعاء الرسول بينكم كدعاء",
  "تبارك الذي إن شاء جعل لك خيرا من ذلك",
  "وقال الذين لا يرجون لقاءنا لولا أنزل",
  "وقال الذين كفروا لولا نزل عليه القرآن جملة",
  "ألم تر إلى ربك كيف مد الظل",
  "تبارك الذي جعل في السماء بروجا",
  "طسم تلك آيات الكتاب المبين لعلك باخع نفسك",
  "قال فرعون وما رب العالمين",
  "قالوا لا ضير إنا إلى ربنا منقلبون",
  "الذي خلقني فهو يهدين",
  "قالوا أنؤمن لك واتبعك الأرذلون",
  "أتتركون في ما هاهنا آمنين",
  "أوفوا الكيل ولا تكونوا من المخسرين",
  "وما أهلكنا من قرية إلا لها منذرون",
  "وإنك لتلقى القرآن من لدن حكيم عليم",
  "حتى إذا أتوا على واد النمل قالت نملة",
  "قال سننظر أصدقت أم كنت من الكاذبين",
  "قال نكروا لها عرشها",
  "قل الحمد لله وسلام على عباده الذين اصطفى",
  "وقال الذين كفروا إذا كنا ترابا وآباؤنا",
  "وإذا وقع القول عليهم أخرجنا لهم دابة",
  "طسم تلك آيات الكتاب المبين نتلو عليك",
  "وحرمنا عليه المراضع من قبل",
  "وجاء رجل من أقصا المدينة يسعى",
  "فلما قضى موسى الأجل وسار بأهله",
  "وقال فرعون يا أيها الملأ ما علمت لكم",
  "ولقد وصلنا لهم القول لعلهم يذكرون",
  "أفمن وعدناه وعدا حسنا فهو لاقيه",
  "إن قارون كان من قوم موسى",
  "تلك الدار الآخرة نجعلها للذين لا يريدون",
  "ووصينا الإنسان بوالديه حسنا وإن جاهداك",
  "وإبراهيم إذ قال لقومه اعبدوا الله واتقوه",
  "فآمن له لوط",
  "وإلى مدين أخاهم شعيبا",
  "ولا تجادلوا أهل الكتاب إلا بالتي هي أحسن",
  "وكأين من دابة لا تحمل رزقها الله يرزقها وإياكم",
  "أولم يسيروا في الأرض فينظروا كيف",
  "ومن آياته خلق السماوات والأرض واختلاف",
  "منيبين إليه واتقوه وأقيموا الصلاة",
  "ظهر الفساد في البر والبحر",
  "الله الذي خلقكم من ضعف",
  "ولقد آتينا لقمان الحكمة أن أشكر لله",
  "ومن يسلم وجهه لله وهو محسن",
  "وإذا غشيهم موج كالظلل دعوا الله",
  "قل يتوفاكم ملك الموت الذي وكل بكم",
  "ولقد آتينا موسى الكتاب فلا تكن في مرية",
  "النبيء أولى بالمؤمنين من أنفسهم",
  "وإذ قالت طائفة منهم يا أهل يثرب",
  "يحسبون الأحزاب لم يذهبوا",
  "يا أيها النبيء قل لأزواجك إن كنتن تردن",
  "إن المسلمين والمسلمات والمؤمنين",
  "ما كان محمد أبا أحد من رجالكم",
  "ترجي من تشاء منهن",
  "لا جناح عليهن في آباءهن ولا أبنائهن",
  "لئن لم ينته المنافقون والذين في قلوبهم",
  "إنا عرضنا الأمانة على السماوات والأرض",
  "وقال الذين كفروا هل ندلكم على رجل",
  "لقد كان لسبإ في مساكنهم آية",
  "قل من يرزقكم من السماوات والأرض قل الله",
  "وما أرسلنا في قرية من نذير إلا قال مترفوها",
  "قل إنما أعظكم بواحدة",
  "يا أيها الناس إن وعد الله حق",
  "يا أيها الناس أنتم الفقراء إلى الله",
  "والذي أوحينا إليك من الكتاب هو الحق",
  "قل أرأيتم شركاءكم الذين تدعون",
  "يس والقرآن الحكيم",
  "وما أنزلنا على قومه من بعده من جند",
  "وإذا قيل لهم اتقوا ما بين أيديكم",
  "ألم أعهد إليكم يا بني آدم",
  "أولم ير الإنسان أنا خلقناه من نطفة",
  "احشروا الذين ظلموا وأزواجهم",
  "قال قائل منهم إني كان لي قرين",
  "وإن من شيعته لإبراهيم",
  "ولقد مننا على موسى وهارون",
  "فنبذناه بالعراء وهو سقيم",
  "ص والقرآن ذي الذكر",
  "وهل أتاك نبأ الخصم إذ تسورا المحراب",
  "ووهبنا لداوود سليمان",
  "وعندهم قاصرات الطرف أتراب",
  "قال فالحق والحق أقول",
  "وإذ مس الإنسان ضر دعا ربه منيبا إليه",
  "أفمن حق عليه كلمة العذاب",
  "فمن أظلم ممن كذب على الله وكذب بالصدق",
  "أم اتخذوا من دون الله شفعاء",
  "قل يا عبادي الذي أسرفوا على أنفسهم",
  "وما قدروا الله حق قدره والأرض جميعا",
  "حم تنزيل الكتاب من الله العزيز العليم",
  "إن الذين كفروا ينادون لمقت الله أكبر",
  "أولم يسيروا في الأرض فينظروا كيف",
  "وقال الذي آمن يا قوم إني أخاف عليكم",
  "ويا قومي مالي أدعوكم إلى النجاة",
  "ولقد آتينا موسى الهدى وأورثنا",
  "قل إني نهيت أن أعبد الذي تدعون",
  "الله الذي جعل لكم الأنعام لتركبوا منها",
  "قل أإنكم لتكفرون بالذي خلق الأرض في يومين",
  "وأما ثمود فهديناهم فاستحبوا العمى",
  "وقال الذين كفروا لا تسمعوا لهذا القرآن",
  "فإن استكبروا فالذين عند ربك يسبحون",
  "إليه يرد علم الساعة",
  "حم عسق كذلك يوحي إليك",
  "شرع لكم من الدين ما وصى به نوحا",
  "أم لهم شركاء شرعوا لهم من الدين",
  "أو يوبقهن بما كسبوا ويعف عن كثير",
  "ولمن صبر وغفر إن ذلك لمن عزم الأمور",
  "وما كان لبشر أن يكلمه الله إلا وحيا",
  "والذي خلق الأزواج كلها",
  "قل أولو جئتكم بأهدى مما وجدتم عليه آباءكم",
  "فاستمسك بالذي أوحي إليك",
  "ولما جاء عيسى بالبينات قال قد جئتكم",
  "وهو الذي في السماء إله وفي الأرض إله",
  "كم تركوا من جنات وعيون وزروع",
  "إن المتقين في مقام أمين",
  "هذا هدى والذين كفروا بآيات ربهم",
  "أفرأيت من اتخذ إلهه هواه",
  "حم تنزيل الكتاب من الله العزيز الحكيم",
  "ووصينا الإنسان بوالديه حسنا",
  "واذكر أخا عاد إذ أنذر قومه بالأحقاف",
  "أولم يروا أن الله الذي خلق السماوات",
  "أفلم يسيروا في الأرض فينظروا كيف",
  "ويقول الذين آمنوا لولا نزلت سورة",
  "يا أيها الذين آمنوا أطيعوا الله وأطيعوا الرسول",
  "إنا أرسلناك شاهدا ومبشرا ونذيرا",
  "لقد رضي الله عن المؤمنين إذ يبايعونك",
  "لقد صدق الله رسوله الرؤيا بالحق",
  "يا أيها الذين آمنوا لا تقدموا بين يدي الله",
  "وإن طائفتان من المؤمنين اقتتلوا",
  "قالت الأعراب آمنا",
  "أفلم ينظروا إلى السماء فوقهم",
  "قال قرينه ربنا ما أطغيته",
  "والذاريات ذروا",
  "قال فما خطبكم أيها المرسلون",
  "وما خلقت الجن والإنس إلا ليعبدون",
  "ويطوف عليهم غلمان لهم كأنهم لؤلؤ مكنون",
  "وإن يروا كسفا من السماء ساقطا",
  "وكم من ملك في السماوات لا تغني شفاعتهم",
  "هذا نذير من النذر الأولى",
  "كذبت عاد فكيف كان عذابي ونذر",
  "ولقد جا آل فرعون النذر",
  "الرحمن علم القرآن",
  "يا معشر الجن والإنس إن استطعتم",
  "هل جزاء الإحسان إلا الإحسان",
  "وأصحاب اليمين",
  "فلا أقسم بمواقع النجوم",
  "آمنوا بالله ورسوله وأنفقوا مما جعلكم",
  "ألم يأن للذين آمنوا أن تخشع قلوبهم",
  "ما أصاب من مصيبة في الأرض",
  "قد سمع الله قول التي تجادلك في زوجها",
  "ألم تر إلى الذين نهوا عن النجوى",
  "ألم تر إلى الذين تولوا قوما غضب الله عليهم",
  "ما قطعتم من لينة أو تركتموها قائمة",
  "ألم تر إلى الذين نافقوا يقولون لإخوانهم",
  "يا أيها الذين آمنوا لا تتخذوا عدوي",
  "عسى الله أن يجعل بينكم وبين الذين عاديتم",
  "سبح لله ما في السماوات وما في الأرض",
  "يسبح لله ما في السماوات وما في الأرض",
  "يا أيها الذين آمنوا إذا نودي للصلاة",
  "يا أيها الذين آمنوا لا تلهكم أموالكم ولا أولادكم",
  "زعم الذين كفروا أن لن يبعثوا",
  "يا أيها الذين آمنوا إن من أزواجكم",
  "اسكنوهن من حيث سكنتم من وجدتكم",
  "الله الذي خلق سبع سماوات ومن الأرض مثلهن",
  "يا أيها الذين آمنوا توبوا إلى الله",
  "تبارك الذي بيده الملك",
  "ألم يروا إلى الطير فوقهم صافات",
  "فطاف عليها طائف من ربك",
  "فاصبر لحكم ربك ولا تكن كصاحب الحوت",
  "فلا أقسم بما تبصرون وما لا تبصرون",
  "إن الإنسان خلق هلوعا",
  "فلا أقسم برب المشارق والمغارب",
  "ألم تروا كيف خلق الله سبع سماوات طباقا",
  "قل أوحي إلى أنه استمع نفر من الجن",
  "وأن المساجد لله فلا تدعو مع الله أحدا",
  "إن ربك يعلم أنك تقوم أدنى من ثلي الليل",
  "وما جعلنا أصحاب النار إلا ملائكة",
  "لا أقسم بيوم القيامة",
  "هل أتى على الإنسان حين من الدهر",
  "ويطوف عليهم ولدان مخلدون",
  "ألم نهلك الأولين",
  "عم يتساءلون",
  "والنازعات غرقا",
  "يسألونك عن الساعة أيان مرساها",
  "فإذا جاءت الصاخة",
  "فلا أقسم بالخنس",
  "كلا إن كتاب الفجار لفي سجين",
  "فلا أقسم بالشفق",
  "إن بطش ربك لشديد",
  "سبح اسم ربك الأعلى",
  "أفلا ينظرون إلى الإبل كيف خلقت",
  "لا أقسم بهذا البلد",
  "والليل إذا يغشى",
  "ألم نشرح لك صدرك",
  "إنا أنزلناه في ليلة القدر",
  "أفلا يعلم إذا بعثر ما في القبور",
  "ألم تر كيف فعل ربك بأصحاب الفيل"
];
const isAyahTextRight = (text: string, ayahId: AyahId, comparisonType: comparisonType = "exact") => {
  const foundAyahText = qulunData[ayahId - 1]?.aya_text;
  return compareFirstLetters(text, foundAyahText, comparisonType)
}

const checkSurroundingAyahs = (text: string, ayahId: number, comparisonType: comparisonType = "levenshtein"): number | undefined => {
  const maxId = 6236;
  const maxOffset = 25;

  for (let offset = 1; offset <= maxOffset; offset++) {
    const backId = ayahId - offset;
    if (backId >= 1 && isAyahTextRight(text, backId, comparisonType)) {
      return backId;
    }

    const forwardId = ayahId + offset;
    if (forwardId <= maxId && isAyahTextRight(text, forwardId, comparisonType)) {
      return forwardId;
    }
  }

  return undefined;
};

const findAyah = (ayahId: AyahId) => qulunData[ayahId - 1]?.aya_text;


/**************************************PROVIDE A TEXT ARRAY / APPROXIMATE AYAH IDs AND GET THUMUNS IDS... */
const getNewAyahIds = (comparisonType: comparisonType) => {
  return baseIds.map((verseId, index) => {
    if( verseId === 0) return undefined; // Skip if verseId is 0
    const comparisonText = textArray[index]
    if (!comparisonText) return
    const result = isAyahTextRight(comparisonText, verseId,comparisonType) ? verseId : checkSurroundingAyahs(comparisonText, verseId, comparisonType)
    return result
  })
}
interface ICheckAyah {
  id: AyahId
  ayahText: string
  datasetText: string
  thumunIndex:number
}
const unfoundAyahs: ICheckAyah[] = []
const idsAddedByLevenshteinMethod: ICheckAyah[] = []

const exactMatchIds = getNewAyahIds("exact")
const levenshteinIds = getNewAyahIds("levenshtein")


const hybridIds = exactMatchIds.map((id, index) => {
  if (id) return id
  if (levenshteinIds[index]) {
    const foundId = levenshteinIds[index]
    idsAddedByLevenshteinMethod.push({ id: foundId, ayahText: findAyah(foundId), datasetText: textArray[index],thumunIndex:index })
    return foundId
  }
  const defaultId = baseIds[index]
  unfoundAyahs.push({ id: defaultId, ayahText: findAyah(defaultId), datasetText: textArray[index],thumunIndex:index })
})

//this will console the best programmatic copy (it contains exactIds if not found: levenshteinId will be added)
//hybridIds.forEach(e => console.log(e))

//this will console data the Ids that are added via levenshtein method (compare the data with te dataset and modify incorrect Ids)
//idsAddedByLevenshteinMethod.forEach(e => console.log(e))

//this will log the Ids that are not found (neither via exact match nor via Levenshtein method )
//unfoundAyahs.forEach(e => console.log(e)) //logs 0 meaning there all ayahs are matched




                   /* Check using ۞ in the Qaloun dataset */
//the Qaloun dataset has 427 marked thumuns(out of 480)
//check using ۞ in the quran Qaloun dataset (thumuns marked in the qaloun dataset but not added to the list)
/*  const unPickedThumuns = qulunData.filter((ayahObj,index)=>{
  return ayahObj.aya_text.startsWith('۞') && !baseIds.includes(ayahObj.id)
 })
console.log(unPickedThumuns); // getting [] SO all ayahs starting with ۞ are picked up   */

//this will log all thumuns that are picked but not marked in the Qaloun dataset
/*   const pickedWithoutMark = qulunData.filter((ayahObj,index)=>{
  return !ayahObj.aya_text.startsWith('۞') && baseIds.includes(ayahObj.id)
 })
 console.log(pickedWithoutMark); */

 

 /* LOGICAL TEST */
//log each id that is larger then the next id (which is an error)
/* const IdLargerThenNext = baseIds.filter((id,index,array)=>{
  return id <=  array[index-1]
})
console.log(IdLargerThenNext); */

//t log all ids that are very close to each other : A suspect of an error (not always an error bcs some ayahs are large)
/*  const IdCloseToThenNext = baseIds.filter((id,index,array)=>{
  return id -  array[index-1] <=3
})
console.log(IdCloseToThenNext); //All checked */





